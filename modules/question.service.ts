...
const app = new Hono()
  ...
  .post(
    "/:lessonId",
    clerkMiddleware(),
    zValidator("param", z.object({ lessonId: z.string() })),
    zValidator("json", insertQuestionSchema),
    async (c) => {
      const auth = getAuth(c);
    ...
    async (c) => {
      ...
      try {
        ...
        const result = await db.transaction(async (trx) => {
          ...
          const updates: typeof answers = [];
          const inserts: typeof answers = [];
  ...
  .put(
    ...
    async (c) => {
      ...
      const { order, questionId } = c.req.valid("json");
  ...
  .delete(
    ...
    async (c) => {
      ...
      try {
        ...
        return c.json(
          { error: "Failed to delete question and progress records" },
          500,
        );
      }
    },
